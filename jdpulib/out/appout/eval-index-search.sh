...
declare -a DOC_COUNTS=(10000 20000 50000 100000 200000 500000 1000000)
        ...
        for cnt_reqs in ${REQ_COUNT[@]}; do
                for cnt_threads in ${THREADS[@]}; do
                        for cnt_dpus in ${DPUS[@]}; do
                                  ...
                                  perf record -e $EVENT_LIST $JAVA $VM_OPTIONS -cp index-search.jar:dpu.jar application.transplant.index.search.IndexSearchMain DOC_COUNT=$cnt_doc TSK_N=$cnt_reqs DPU_COUNT=$cnt_dpus THREADS=$cnt_threads PROF_CPUDPU_DM 
        ...
        for cnt_reqs in ${REQ_COUNT[@]}; do
                for cnt_threads in ${THREADS[@]}; do
                        for cnt_dpus in ${DPUS[@]}; do
                                  ...
                                  echo "NODES=$cnt_nodes, REQUEST=$cnt_reqs, THREADS=$cnt_threads, DPUS=$cnt_dpus"
                                  sh start_simulator_server.sh $cnt_dpus $cnt_threads &
                                  perf record -e $EVENT_LIST $JAVA $VM_OPTIONS -cp index-search.jar:dpu.jar application.transplant.index.search.IndexSearchMain DOC_COUNT=$cnt_nodes TSK_N=$cnt_reqs DPU_COUNT=$cnt_dpus THREADS=$cnt_threads PROF_CPUDPU_DM > ./record-indexsearch/mov-n${cnt_nodes}-r${cnt_reqs}-t${cnt_threads}-d${cnt_dpus}.txt
                                  perf report --stdio > ./record-indexsearch/n${cnt_nodes}-r${cnt_reqs}-t${cnt_threads}-d${cnt_dpus}.txt
                                  kill %1 2> /dev/null && wait $1 2> dev/null
                                  bash ./kill_all_defunct.sh
                        done
