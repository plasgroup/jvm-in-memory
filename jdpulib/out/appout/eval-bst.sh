    ...
    for cnt_threads in ${THREADS[@]}; do
	  for cnt_nodes in ${NODES_COUNTS[@]}; do
          ...
          perf report --stdio > ./bst-tree/n-${cnt_nodes}-l-${cnt_layer}-d-${cnt_dpus}/[PIM]n-${cnt_nodes}-r${cnt_reqs}-t${cnt_threads}-d${cnt_dpus}-no-batch.txt
				...
				if [ ! -f ./bst-tree/n-${cnt_nodes}-l-${cnt_layer}-d-${cnt_dpus}/[CPU-EXEC]n-${cnt_nodes}-r${cnt_reqs}-d${cnt_dpus}.txt || ! -f ./bst-tree/n-${cnt_nodes}-l-${cnt_layer}-d-${cnt_dpus}/[CPU]n-${cnt_nodes}-r${cnt_reqs}-d${cnt_dpus}.txt ]; then	
				 ### CPU-Only version 
