...
for cnt_nodes in ${NODES_COUNTS[@]}; do
				  ...
				  mkdir -p ./bst-tree/n-$cnt_nodes-l-$cnt_layer-d-$cnt_dpus
                                  $JAVA $VM_OPTIONS -cp bst.jar:dpu.jar Main TYPE=PIM NODES=$cnt_nodes DPU_COUNT=$cnt_dpus CPU_LAYER_COUNT=$cnt_layer IMG_PATH=./bst-tree/n-$cnt_nodes-l-$cnt_layer-d-$cnt_dpus/ NO_SEARCH THREADS=1 BUILD_FROM_IMG=0 SERIALIZE_TREE
