...
public class DPUManagerUPMEM extends DPUManager {
    ...
    public <T> DummyProxy createObject(Class c, Object[] params) {
        if (UPMEM.getConfigurator().isUseSimulator()) {
            ...
            // UPMEM.getInstance().getDPUManager(dpuID).classCacheManager.getClassStructure("framework.pim/algorithm/DPUTreeNode").virtualTable;
        ...
        // we need iterate the class's method table to check whether there exists a
        // method that with the
        // same name of the method we want to call with a parameter list that each
        ...
        if (classCacheManager
            ...
            Dictionary<String, DPUMethodLookupTableItem> stringDPUMethodCacheItemDictionary = classCacheManager.methodCache.cache
                    .get(c.getName().replace(".", "/"));
            ...
            if (sb.charAt(pos) == ')') {
                ...
                if (state == 0) {
                    ...
                    if (ch != ';') {
                        ...
                        try {
                            if (Class.forName(matched.toString().replace("/", "."))
                                    .isAssignableFrom(params[paramIndex].getClass())) {
                                paramIndex++;
                                pos++;
                            }
    ...
    private void printClassCache(int dpuID) {
        DPULookupTableManager.DPUClassLookupTable dpuClassLookupTable = UPMEM.getInstance()
                .getDPUManager(dpuID).classCacheManager.dpuClassLookupTable;
    ...
    private void printMethodCache(int dpuID) {
        DPULookupTableManager.DPUMethodLookupTable dpuMethodLookupTable = UPMEM.getInstance()
                .getDPUManager(dpuID).classCacheManager.methodCache;
        ...
        for (Iterator<String> it = dpuMethodLookupTable.cache.keys().asIterator(); it.hasNext();) {
            ...
            Dictionary<String, DPUMethodLookupTableItem> dpuMethodLookupTableItem = dpuMethodLookupTable.cache.get(key);
