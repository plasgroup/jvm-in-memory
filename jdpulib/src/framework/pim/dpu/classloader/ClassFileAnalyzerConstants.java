package framework.pim.dpu.classloader;

public class ClassFileAnalyzerConstants {
    public static final byte CT_Class = 7;
    public static final byte CT_Fieldref = 9;
    public static final byte CT_Methodref = 10;
    static final byte CT_InterfaceMethodref = 11;
    static final byte CT_String = 8;
    static final byte CT_Integer = 3;
    static final byte CT_Float = 4;
    static final byte CT_Long = 5;
    static final byte CT_Double = 6;
    static final byte CT_NameAndType = 12;
    static final byte CT_Utf8 = 1;
    static final byte CT_MethodHandle = 15;
    static final byte CT_MethodType = 16;
    static final byte CT_InvokeDynamic = 18;


}
